...
export async function POST(request: Request) {
  ...
  // 1. 从数据库检索相关资料（真实数据，绝不编造）
  const [stylesRes, ordersRes, fabricsRes, accessoriesRes, docsRes] =
    await Promise.all([
      supabase
        .from("styles")
        .select("style_no,style_name,customer_style_no,category,status,notes")
        .or(`style_no.ilike.${like},style_name.ilike.${like},customer_style_no.ilike.${like},notes.ilike.${like}`)
        .limit(8),
      supabase
        .from("orders")
        .select("style_no,po_no,colorway,quantity,target_qty,delivery_date,status,current_progress,risk_level,thread_info,fabric_summary")
        .or(`po_no.ilike.${like},style_no.ilike.${like},colorway.ilike.${like},order_no.ilike.${like}`)
        .limit(8),
      supabase
        .from("fabric_info")
        .select("fabric_name,material_code,category,composition,colorway,usage_per_piece,unit_price,supplier,notes")
        .or(`fabric_name.ilike.${like},material_code.ilike.${like},composition.ilike.${like},supplier.ilike.${like},category.ilike.${like}`)
        .limit(8),
      supabase
        .from("accessory_info")
        .select("accessory_name,material_code,spec,colorway,quantity,supplier,tracking_status,order_date,expected_arrival,actual_arrival,notes")
  ...
  if (orders.length) lines.push("【大货单】" + orders.map((o) => `${o.style_no}/${o.po_no}/${o.colorway ?? ""} 数量${o.quantity ?? "?"} 目标${o.target_qty ?? "?"} 货期${o.delivery_date ?? "?"} 状态${o.status ?? ""} 进度:${o.current_progress ?? "无"} 风险:${o.risk_level ?? "无"}`).join("；"));
  if (fabrics.length) lines.push("【面料】" + fabrics.map((f) => `${f.fabric_name}(${f.material_code ?? ""}/${f.category ?? ""}/${f.composition ?? ""}) 颜色${f.colorway ?? ""} 用量${f.usage_per_piece ?? ""} 供应商${f.supplier ?? ""}`).join("；"));
  if (accessories.length) lines.push("【辅料】" + accessories.map((a) => `${a.accessory_name}(${a.material_code ?? ""}/${a.spec ?? ""}) 颜色${a.colorway ?? ""} 供应商${a.supplier ?? ""} 状态:${a.tracking_status ?? ""} 下单${a.order_date ?? "?"} 预计到货${a.expected_arrival ?? "?"} 实际到货${a.actual_arrival ?? "?"} 备注${a.notes ?? ""}`).join("；"));
  if (docs.length) lines.push("【文档】" + docs.map((d) => `${d.file_name}：${String(d.original_text ?? "").slice(0, 300)}`).join("\n"));

  const context = lines.join("\n");
  const hasData = styles.length + orders.length + fabrics.length + accessories.length + docs.length > 0;
